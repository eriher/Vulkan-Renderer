...
layout(location = 2) in vec3	worldSpaceCam; 
layout(location = 3) in vec2 fragTexCoord;
layout(location = 4) in mat4 viewMatrix;
layout(location = 8) in vec3 tangentLightPos;
layout(location = 9) in vec3 tangentViewPos;
layout(location = 10) in vec3 tangentFragPos;
layout(location = 11) in vec3 tangentNormPos;
...
{		
    ...
    vec3 prefilteredColor = textureLod(reflectionMap, TBN * R,  roughness * MAX_REFLECTION_LOD).rgb;
    vec2 brdf  = texture(brdfLUT, vec2(max(dot(TBN * N, TBN * V), 0.0), roughness)).rg;

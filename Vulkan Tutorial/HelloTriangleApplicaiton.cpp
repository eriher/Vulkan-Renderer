...
private:
  ...
  void initVulkan() {
    ...
    //createOffscreenPipeline2();

    setupOffscreenSkybox();
  ...
  static void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
  {
    ...
    if (key == GLFW_KEY_Z) {
      if (action == GLFW_PRESS)
      {
        app->offscreenUbo.view = app->offScreencaptureViews[++app->zcounter%6];
        app->renderOffscreen();
  ...
  void createOffscreenSync() {
    VkSemaphoreCreateInfo semaphoreInfo{};
    semaphoreInfo.sType = VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO;
    if (vkCreateSemaphore(device.device, &semaphoreInfo, nullptr, &offscreenImageAvailableSemaphore) != VK_SUCCESS ||
      vkCreateSemaphore(device.device, &semaphoreInfo, nullptr, &offscreenRenderFinishedSemaphore) != VK_SUCCESS) {
